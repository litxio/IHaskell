...
import IHaskell.Display (displayFromChanEncoded)
import IHaskell.Eval.Widgets (relayWidgetMessages)
import IHaskell.Eval.PtGhci
...
type Interpreter = ReaderT Ghci IO
...
interpret libdir allowedStdin needsSupportLibraries action = do
  ...
  (haveSupportLibraries, importPkgs) <- getDisplayPkgs needsSupportLibraries 
  -- initializeItVariable
  let run = do
        importDisplayModules (if needsSupportLibraries then importPkgs else [])
        action haveSupportLibraries
      extraPkgs = intercalate " " $ ("--package " ++) <$> importPkgs

      cmd = "stack ghci --ghci-options \"-interactive-print=IHaskell.Eval.PtGhci.ihaskellDisplay\" "
            ++extraPkgs
  putStrLn $ "GHCi command: "++cmd
  (ghci, loadMsgs) <- startGhci ("stack ghci --ghci-options \"-interactive-print=IHaskell.Eval.PtGhci.ihaskellDisplay\" "++extraPkgs) Nothing (\strm s -> print (strm, s))
  runReaderT run ghci
...
getIHaskellPackages :: IO [String]
getIHaskellPackages = words <$> readProcess "stack" args ""
  where 
    args = ["exec", "--", "ghc-pkg", "list", "--simple-output", "--names-only", "ihaskell-*"]
...
-- library is available, and the list of packages we should have GHCi load.
getDisplayPkgs :: Bool -> IO (Bool, [String])
getDisplayPkgs importSupportLibraries = do
...
-- #endif
      ...
      --               , isAlpha x ]
  displayPkgs <- liftIO $ getIHaskellPackages 

  -- let hasIHaskellPackage = not $ null $ filter (== iHaskellPkgName) packageNames
  let hasIHaskellPackage = not $ null $ filter (== iHaskellPkgName) displayPkgs
  return (hasIHaskellPackage, displayPkgs)


importDisplayModules :: [String] -> Interpreter ()
importDisplayModules displayPkgs = do
  ...
  ghci <- ask
  liftIO $ execStream ghci $ "import IHaskell.Eval.PtGhci ()"
  liftIO $ for_ displayImports $ \s -> do
    putStrLn s
    execStream ghci s
...
initializeItVariable =
  ...
  return () -- void $ execStmt "let it = ()" execOptions
...
evaluate kernelState code output widgetHandler = do
  ...
  ghci <- ask
  res <- evalToResult code output
  liftIO $ output res Success
  let updated = kernelState
  ...
  where
    ...
    --                   let getEncodedDisplays = Right displayFromChanEncoded
...
flushWidgetMessages state evalmsgs widgetHandler = do
  ...
  let extracted = Right relayWidgetMessages
  liftIO $
    case extracted of
      Left err -> do
